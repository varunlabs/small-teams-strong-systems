...
**When execution becomes abundant, judgment becomes the limiting factor.** Durable scale is not a function of size. It is a function of design.
...
build/               # Build scripts (EPUB, DOCX, PDF)
published/
└── v1/              # Amazon KDP submitted files (EPUB + covers)
drafts/
└── v2/              # v2 in progress — build output (EPUB, DOCX, PDF)
...
Files in this repo:
- [`published/v1/`](published/v1/) — v1 as submitted to Amazon (EPUB + covers)
- [`drafts/v2/`](drafts/v2/) — v2 in progress (EPUB, DOCX, PDF)
...
- XeLaTeX (via [TeX Live](https://tug.org/texlive/) or [MiKTeX](https://miktex.org/)) — for PDF only
- Fonts: Georgia, Arial, Courier New (standard system fonts on Windows and macOS)

**PowerShell (Windows):**

```powershell
.\build\build_epub.ps1     # EPUB
.\build\build_docx.ps1     # DOCX
.\build\build_pdf.ps1      # PDF (requires XeLaTeX)
```
...
./build/build_pdf.sh       # PDF (requires XeLaTeX)
```

Output goes to `drafts/v2/`.

---

## About the author

**Varun Kumar Siddaraju** is a developer, software architect, product manager, and founder with over a decade of experience building spatial and applied-AI systems across research, enterprise, and startup contexts. His work spans extended reality, context-aware computing, and frontier technology infrastructure.

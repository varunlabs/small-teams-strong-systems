...
# Refresh PATH to include newly installed pandoc
...
Write-Host "EPUB exit code: $exitCode"

if ($exitCode -eq 0) {
  Write-Host "EPUB built successfully: $OUT/SmallTeamsStrongSystems.epub"
  Get-Item "$OUT/SmallTeamsStrongSystems.epub" | Format-List Length
} else {
  Write-Host "EPUB build failed with exit code $exitCode"
  exit $exitCode
}
